...
from scripts.data import load_scGPT
...
GENE_PATH = DATA_DIR / "scGPT_train_val_embeddings.h5ad"
...
OUT_DIR = Path("/dtu/blackhole/19/221977/Results/MLP_on_scGPT_2")
...
def main_mlp():
    ...
    genes_tensor, isoform_tensor = load_scGPT(
        GENE_PATH,
        ISOFORM_PATH,
    ...
    print(f"Embeddings tensor shape: {genes_tensor.shape}")
    print(f"Isoform tensor shape: {isoform_tensor.shape}")
    ...
    train_dataset = TensorDataset(genes_tensor[train_idx], isoform_tensor[train_idx])
    val_dataset = TensorDataset(genes_tensor[val_idx], isoform_tensor[val_idx])
    ...
    model = MLP(
        ...
        out_dim=isoform_tensor.shape[1],
        dropout=DROPOUT,
    ...
    mlp_out = OUT_DIR / "scGPT_MLP"
    mlp_out.mkdir(parents=True, exist_ok=True)

    checkpoint_path = mlp_out / "scGPT_MLP_weights.pth"
    torch.save(
        {
    ...
    plot_history(history, mlp_out / "scGPT_MLP_Training.png")
    plot_residuals(history['val_preds'], history['val_targets'], mlp_out / "scGPT_MLP_Evaluation.png")
